...
if [ $# -lt 1 ]; then
        ...
        echo "e.g. $0 ../../src"
        echo "Env vars FRENCHD and FRENCHCLI may be used to specify the exact binaries used"
        exit 1
fi

set -f

FRENCHD=${FRENCHD:-${1}/bitcoind}
CLI=${FRENCHCLI:-${1}/bitcoin-cli}
...
$FRENCHD -zapwallettxes=1 $B1ARGS &
B1PID=$!
$FRENCHD -zapwallettxes=2 $B2ARGS &
B2PID=$!
