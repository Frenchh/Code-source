...
void MultiSendDialog::updateCheckBoxes()
{
    ...
    ui->multiSendFrenchnodeCheckBox->setChecked(pwalletMain->fMultiSendFrenchnodeReward);
}
...
void MultiSendDialog::on_viewButton_clicked()
{
    ...
    if (pwalletMain->isMultiSendEnabled()) {
        if (pwalletMain->fMultiSendStake)
            ...
            strMultiSendPrint += "MultiSend Active for Frenchnode Rewards\n";
    } else
...
void MultiSendDialog::on_activateButton_clicked()
{
    ...
    if (pwalletMain->vMultiSend.size() < 1)
    ...
    else if (!(ui->multiSendStakeCheckBox->isChecked() || ui->multiSendFrenchnodeCheckBox->isChecked())) {
        ...
        pwalletMain->fMultiSendFrenchnodeReward = ui->multiSendFrenchnodeCheckBox->isChecked();

        CWalletDB walletdb(pwalletMain->strWalletFile);
        if (!walletdb.WriteMSettings(pwalletMain->fMultiSendStake, pwalletMain->fMultiSendFrenchnodeReward, pwalletMain->nLastMultiSendHeight))
